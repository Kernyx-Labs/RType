...
bool Screens::connectTcp() {
  try {
    ...
    if (!_username.empty()) {
      std::memcpy(helloMsg.data() + sizeof(hello), _username.data(),
                  _username.size());
    }
    ...
    std::array<char,
               sizeof(rtype::net::Header) + sizeof(rtype::net::HelloAckPayload)>
        ackBuf{};
    ...
    auto *ackPayload = reinterpret_cast<rtype::net::HelloAckPayload *>(
        ackBuf.data() + sizeof(rtype::net::Header));
    _udpPort = ackPayload->udpPort;

    logMessage("TCP handshake complete, UDP port: " + std::to_string(_udpPort),
               "INFO");
    return true;
...
void Screens::ensureNetSetup() {
  ...
  g.server =
      *resolver
           .resolve(asio::ip::udp::v4(), _serverAddr, std::to_string(_udpPort))
           .begin();
