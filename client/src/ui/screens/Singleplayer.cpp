...
void Screens::drawSingleplayer(ScreenState &screen,
                               SingleplayerForm & /*form*/) {
  ...
  if (!_singleplayerActive) {
    titleCentered("Singleplayer", (int)(h * 0.10f), (int)(h * 0.08f), RAYWHITE, font);
  }
  ...
  if (!_singleplayerActive) {
    ...
    Rectangle startBtn{(float)btnX, (float)btnY, (float)btnWidth,
                       (float)btnHeight};
    if (button(startBtn, "Start", baseFont, BLACK, (Color){120, 200, 120, 255},
               (Color){150, 230, 150, 255}, font)) {
    ...
    // Controls hint
    titleCentered("Controls: WASD/Arrows to move. ESC to pause.",
                  (int)(h * 0.70f), baseFont, LIGHTGRAY, font);
  } else {
    ...
    if (_gameOver) {
      ...
      titleCentered("Game Over", (int)(h * 0.35f), (int)(h * 0.10f), RAYWHITE, font);
      int btnWidth = (int)(w * 0.22f);
      int btnHeight = (int)(h * 0.08f);
      int btnGap = (int)(w * 0.02f);
      ...
      if (button({(float)btnX, (float)btnY, (float)btnWidth, (float)btnHeight},
                 "Restart", baseFont, BLACK, (Color){180, 180, 220, 255},
                 (Color){210, 210, 240, 255}, font)) {
        shutdownSingleplayerWorld();
        initSingleplayerWorld();
        _singleplayerActive = true;
        return;
      }
      if (button({(float)(btnX + btnWidth + btnGap), (float)btnY,
                  (float)btnWidth, (float)btnHeight},
                 "Exit", baseFont, BLACK, (Color){200, 80, 80, 255},
      ...
      titleCentered("Paused", (int)(h * 0.35f), (int)(h * 0.10f), RAYWHITE, font);
      int btnWidth = (int)(w * 0.22f);
      int btnHeight = (int)(h * 0.08f);
      int btnGap = (int)(w * 0.02f);
      int btnY = (int)(h * 0.55f);
      int btnX = (w - (btnWidth * 2 + btnGap)) / 2;
      if (button({(float)btnX, (float)btnY, (float)btnWidth, (float)btnHeight},
                 "Resume", baseFont, BLACK, LIGHTGRAY, GRAY, font)) {
        _spPaused = false;
      }
      if (button({(float)(btnX + btnWidth + btnGap), (float)btnY,
                  (float)btnWidth, (float)btnHeight},
...
void Screens::initSingleplayerWorld() {
  ...
  _spWorld->emplace<rt::components::Position>(
      (rt::ecs::Entity)player, rt::components::Position{100.f, 100.f});
  _spWorld->emplace<rt::components::Controller>((rt::ecs::Entity)player,
                                                rt::components::Controller{});
  _spWorld->emplace<rt::components::Player>((rt::ecs::Entity)player,
                                            rt::components::Player{});
  _spWorld->emplace<rt::components::Size>((rt::ecs::Entity)player,
                                          rt::components::Size{24.f, 16.f});
  _spPlayer = player;
...
void Screens::updateSingleplayerWorld(float dt) {
  ...
  if (auto *c = _spWorld->get<rt::components::Controller>(_spPlayer))
    c->bits = bits;
  ...
  _spSpawnTimer += dt;
  _spShootCooldown -= dt;
  if (_spShootCooldown < 0.f)
    _spShootCooldown = 0.f;
  // Tick invincibility frames
  if (_spHitIframes > 0.f) {
    _spHitIframes -= dt;
    if (_spHitIframes < 0.f)
      _spHitIframes = 0.f;
  }
  ...
  if (_spInvincibleTimer > 0.f) {
    _spInvincibleTimer -= dt;
    if (_spInvincibleTimer < 0.f)
      _spInvincibleTimer = 0.f;
  }
  // Tick infinite fire
  if (_spInfiniteFireTimer > 0.f) {
    _spInfiniteFireTimer -= dt;
    if (_spInfiniteFireTimer < 0.f)
      _spInfiniteFireTimer = 0.f;
  }
  ...
  // Player shooting: Space creates a bullet with cooldown; during InfiniteFire,
  // ignore heat gating
  if (shootHeld && _spShootCooldown <= 0.f &&
      (_spHeat > 0.f || _spInfiniteFireTimer > 0.f)) {
    if (auto *pp = _spWorld->get<rt::components::Position>(_spPlayer)) {
      ...
      _spBullets.push_back(
          {bx, by, _spBulletSpeed, 0.f, _spBulletW, _spBulletH});
  ...
  if (!_gameOver && !_spBossActive && _spSpawnTimer >= _spNextSpawnDelay &&
      _spEnemies.size() < _spEnemyCap) {
    _spSpawnTimer = 0.f;
    // choose formation cyclically to keep variety while still randomizing
    // timing
    ...
    case 0:
      ...
      break;
    case 1:
      spSpawnSnake(6, y, 70.f, 2.2f, 36.f);
      break;
    case 2:
      spSpawnTriangle(5, y, 36.f);
      break;
    case 3:
      spSpawnDiamond(4, y, 36.f);
      break;
    }
    spScheduleNextSpawn();
  }
  ...
  for (auto it = _spEnemies.begin(); it != _spEnemies.end();) {
    ...
    case SpFormationKind::Snake: {
      ...
      if (!_gameOver)
        bitsAI |= (s > 0.f ? kUp : 0);
      // small downward occasionally to prevent sticking at top if extremely
      // high
      if (!_gameOver && s <= 0.f && ((en.index + (int)(t)) % 3 == 0))
        bitsAI |= kDown;
      break;
    }
  ...
  // Enemy shooting: enemies with shooter=true fire towards the player with some
  // inaccuracy
  if (!_gameOver) {
    // Player center
    float px = 0.f, py = 0.f;
  ...
  if (!_gameOver)
    for (std::size_t i = 0; i < _spBullets.size();) {
      ...
      if (destroyBullet)
        _spBullets.erase(_spBullets.begin() + (long)i);
      else
        ++i;
    }
  ...
  if (!_gameOver) {
    ...
    for (std::size_t i = 0; i < _spEnemyBullets.size();) {
      ...
      bool destroyBullet = false;
      // Offscreen
      if (b.x + b.w < -40.f || b.x > screenW + 60.f || b.y + b.h < -40.f ||
          b.y > screenH + 60.f)
        destroyBullet = true;
      // Collide with player
      if (!destroyBullet) {
        if (auto *pp = _spWorld->get<rt::components::Position>(_spPlayer)) {
          ...
          if (hit) {
            // If shield or i-frames, ignore life loss but still remove the
            // bullet
            if (_spInvincibleTimer <= 0.f && _spHitIframes <= 0.f &&
                _playerLives > 0) {
      ...
      if (destroyBullet)
        _spEnemyBullets.erase(_spEnemyBullets.begin() + (long)i);
      else
        ++i;
    }
  }
  ...
  // Clamp player inside the game area (stay within screen and above the bottom
  // HUD bar)
...
void Screens::drawSingleplayerWorld() {
  ...
  if (_spBossActive && _spBossId != 0) {
    if (auto *bp = _spWorld->get<rt::components::Position>(_spBossId)) {
      ...
      DrawRectangleLines((int)rect.x, (int)rect.y, (int)rect.width,
                         (int)rect.height, (Color){220, 200, 240, 255});
    }
  }
  ...
  for (int i = 0; i < total; ++i) {
    Color c = i < _playerLives ? (Color){100, 220, 120, 255}
                               : (Color){80, 80, 80, 180};
    DrawRectangle(startX + i * (sqSize + gap), h - barH + margin, sqSize,
                  sqSize, c);
  }

  // Draw overheat bar at center bottom: red depleting bar while firing,
  // recharges when idle
...
void Screens::spHandleScoreThresholdSpawns(int screenW) {
  while (_score >= _spNextPowerupScore) {
    ...
    if (slot == 0)
      type = SpPowerupType::Life;
    else if (slot == 1)
      type = SpPowerupType::Invincibility;
    else if (slot == 2)
      type = SpPowerupType::ClearBoard;
    else
      type = SpPowerupType::InfiniteFire;
...
void Screens::spUpdatePowerups(float dt) {
  ...
  for (std::size_t i = 0; i < _spPowerups.size();) {
    ...
    if (rectCircleHit(pu.x, pu.y, pu.radius)) {
      ...
      case SpPowerupType::ClearBoard: {
        ...
        for (std::size_t ei = 0; ei < _spEnemies.size();) {
          ...
          if (auto *ep = _spWorld->get<rt::components::Position>(en.id)) {
            float ex = ep->x, ey = ep->y, ew = 24.f, eh = 16.f;
            bool onScreen = !(ex + ew < 0.f || ex > (float)screenW ||
                              ey + eh < 0.f || ey > (float)screenH);
            if (onScreen) {
              _spWorld->destroy(en.id);
              _spEnemies.erase(_spEnemies.begin() + (long)ei);
    ...
    if (pu.x + pu.radius < -20.f)
      remove = true;
    if (remove)
      _spPowerups.erase(_spPowerups.begin() + (long)i);
    else
      ++i;
  }
}
...
void Screens::spSpawnLine(int count, float y) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    _spWorld->emplace<rt::components::Position>((rt::ecs::Entity)e,
                                                rt::components::Position{x, y});
    _spWorld->emplace<rt::components::Enemy>((rt::ecs::Entity)e,
                                             rt::components::Enemy{});
    _spWorld->emplace<rt::components::AiController>(
        (rt::ecs::Entity)e, rt::components::AiController{});
    _spWorld->emplace<rt::components::Size>((rt::ecs::Entity)e,
                                            rt::components::Size{24.f, 16.f});
    SpEnemy info{e,          SpFormationKind::Line, i,  y, spacing, 0.f, 0.f,
                 _spElapsed, (float)i * spacing,    0.f};
...
void Screens::spSpawnSnake(int count, float y, float amplitude, float frequency,
                           float spacing) {
  float startX = (float)GetScreenWidth() + 40.f;
  std::uniform_int_distribution<int> chance(0, 99);
  for (int i = 0; i < count; ++i) {
...
void Screens::spSpawnTriangle(int rows, float y, float spacing) {
  ...
  for (int col = 0; col < rows; ++col) {
    ...
    for (int r = 0; r < count; ++r) {
      ...
      _spWorld->emplace<rt::components::Position>(
          (rt::ecs::Entity)e,
          rt::components::Position{startX + localX, y + localY});
      _spWorld->emplace<rt::components::Enemy>((rt::ecs::Entity)e,
                                               rt::components::Enemy{});
      _spWorld->emplace<rt::components::AiController>(
          (rt::ecs::Entity)e, rt::components::AiController{});
      _spWorld->emplace<rt::components::Size>((rt::ecs::Entity)e,
                                              rt::components::Size{24.f, 16.f});
      SpEnemy info{e,       SpFormationKind::Triangle,
                   idx++,   y,
                   spacing, 0.f,
                   0.f,     _spElapsed,
                   localX,  localY};
...
void Screens::spSpawnDiamond(int rows, float y, float spacing) {
  ...
  for (int col = 0; col < rows; ++col) {
    ...
    for (int r = 0; r < count; ++r) {
      ...
      _spWorld->emplace<rt::components::Size>((rt::ecs::Entity)e,
                                              rt::components::Size{24.f, 16.f});
      SpEnemy info{e,       SpFormationKind::Diamond,
                   idx++,   y,
                   spacing, 0.f,
                   0.f,     _spElapsed,
                   localX,  localY};
...
void Screens::spSpawnBoss() {
  ...
  auto e = _spWorld->create();
  _spWorld->emplace<rt::components::Position>(
      (rt::ecs::Entity)e, rt::components::Position{spawnX, y});
  _spWorld->emplace<rt::components::Enemy>((rt::ecs::Entity)e,
                                           rt::components::Enemy{});
  _spWorld->emplace<rt::components::AiController>(
      (rt::ecs::Entity)e, rt::components::AiController{});
  _spWorld->emplace<rt::components::Size>(
      (rt::ecs::Entity)e, rt::components::Size{_spBossW, _spBossH});
