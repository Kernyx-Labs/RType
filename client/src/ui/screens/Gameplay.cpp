...
void Screens::drawGameplay(ScreenState &screen) {
  ...
  // Build input from arrows OR WASD, gate within playable area based on self
  // pos
  ...
  if (self) {
    float dstX = self->x;
    float dstY = self->y;
    if (wantLeft && dstX > 0.0f)
      bits |= rtype::net::InputLeft;
    if (wantRight && dstX + pW < (float)w)
      bits |= rtype::net::InputRight;
    if (wantUp && dstY > (float)playableMinY)
      bits |= rtype::net::InputUp;
    if (wantDown && dstY + pH < (float)playableMaxY)
  ...
  // Simple overheat UI mechanic (client-side only): drains while firing, regens
  // when not
  if (wantShoot) {
  ...
  for (auto &e : _entities) {
    ...
    if (e.type == 1) {
      ...
      float x = e.x, y = e.y;
      if (y < playableMinY)
        y = (float)playableMinY;
      if (y + pH > playableMaxY)
        y = (float)(playableMaxY - pH);
      if (x < 0)
        x = 0;
      if (x + pW > w)
        x = (float)(w - pW);
      ...
      // Sprite rendering
      int rowIndex = 0;
      if (auto it = _spriteRowById.find(e.id); it != _spriteRowById.end()) {
        rowIndex = it->second;
      }

      if (_sheetLoaded && _frameW > 0 && _frameH > 0) {
        // Calculate tilt column: 2=Straight, 0/1=Down, 3/4=Up
        int colIndex = 2;
        if (e.vy < -50.f)
          colIndex = 4; // Max Up
        else if (e.vy < -10.f)
        ...
        DrawRectangle((int)x, (int)y, (int)pW, (int)pH,
                      (Color){100, 200, 255, 255});
      }

      // Draw invincibility shield if player has it (check based on entity color
      // alpha or a special marker) Since we can't easily track server-side
      // invincibility on client, we'll skip the shield visual Or we could add
      ...
      Color fill = {r, g, b, a};
      Color line = {(std::uint8_t)std::min(255, r + 40),
                    (std::uint8_t)std::min(255, g + 40),
                    (std::uint8_t)std::min(255, b + 40), a};
