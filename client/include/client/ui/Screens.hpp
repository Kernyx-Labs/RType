...
public:
  ...
  // Gracefully leave any active multiplayer session (sends Disconnect, closes
  // socket)
  void leaveSession();
...
private:
  ...
  void spSpawnSnake(int count, float y, float amplitude, float frequency,
                    float spacing);
  ...
  // Overheat mechanic (center bar): drains while holding fire, locks shooting
  // at 0, recovers when not firing
  ...
  int _spNextPowerupScore =
      1500;                     // next score threshold for spawning a power-up
  ...
  // Boss state (spawns once when reaching a score threshold, moves in from
  // right then holds at right side)
  ...
  float _spBossSpeedY = 100.f; // effective speed is governed by Ai bits; value
                               // used for tuning if needed
  ...
  // Parse a single UDP datagram payload according to our protocol and update
  // local state
  ...
  // Entity reconciliation buffers: avoid dropping entities on transient packet
  // loss or truncation
  std::unordered_map<unsigned, PackedEntity>
      _entityById; // id -> last known entity state
  std::unordered_map<unsigned, int>
      _missedById; // id -> consecutive snapshots missed
  std::unordered_map<unsigned, double>
      _lastSeenAt; // id -> last time seen in a snapshot (seconds)
  int _missThreshold =
      3; // safeguard: remove only if both miss-count and time exceed
  double _expireSecondsEnemy =
      2.0; // enemies expire after not seen for this time
